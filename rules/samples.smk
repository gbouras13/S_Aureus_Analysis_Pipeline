...
"""
Function for parsing the Prophages from the prophage directory
"""


def prophagesFromDirectory(dir):
    """Parse prophages from a directory"""
    outDict = {}
    # https://stackoverflow.com/questions/11860476/how-to-unnest-a-nested-list
    prophages= glob_wildcards(os.path.join(dir,'{prophage}.fasta'))
    prophages2 = chain(*samples)
    for prophages in prophages2:
        outDict[prophage] = {}
        fasta = os.path.join(dir,f'{prophage}.fasta')
        if os.path.isfile(fasta):
            outDict[prophage]['fasta'] = fasta
        else:
            sys.stderr.write("\n"
...
def parseProphages(readFileDir):
    """Parse prophages from directory"""
    if os.path.isdir(readFileDir):
        sampleDict = prophagesFromDirectory(readFileDir)
    else:
        sys.stderr.write("\n"
    ...
    if len(sampleDict.keys()) == 0:
        sys.stderr.write("\n"
                         "    FATAL: We could not detect any prophages at all.\n"
                         "\n")
        sys.exit(1)
    return sampleDict
